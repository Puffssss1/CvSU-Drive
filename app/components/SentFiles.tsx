...
const FolderList = () => {
  ...
  useEffect(() => {
    const fetchFolders = async () => {
      ...
      const { data, error } = await supabase
        .from('file_metadata')
        .select('id, file_name, file_url, uploaded_at, uploaded_by, sent_by')
  ...
  // Filter the folders based on the selected time range and sent_by value
  const filteredFolders = folders.filter((folder) => {
    // Only show files that have been sent to the current user
    if (!folder.sent_by) return false; // Only include files that have a value in sent_by
  ...
  return (
    <Box sx={{ padding: 3, marginLeft: '220px', minWidth: '600px', maxWidth: '1200px' }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Typography variant="h5">Files Sent to You</Typography>
