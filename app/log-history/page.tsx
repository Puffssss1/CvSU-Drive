...
interface Folder {
    ...
    uploaded_at: string;
    uploaded_by: string;
    isShown: boolean; // Add isShown field
}
...
export default function LogHistory() {
        ...
        const fetchData = async () => {
            try {
                const { data, error } = await supabase
                    .from('file_metadata')
                    .select('*'); // Fetch all fields including isShown
    ...
    const handleDeleteLog = async (folderId: string) => {
        const { error } = await supabase
            .from('file_metadata')
            .update({ isShown: false }) // Update isShown to false
            .eq('id', folderId);
        ...
        if (error) {
            ...
            setRows((prevRows) =>
                prevRows.map((row) =>
                    row.id === folderId ? { ...row, isShown: false } : row
                )
            );
        }
    };
                    ...
                    {loading ? (
                        ...
                        <div>
                            ...
                            >
                                <TableHead>
                                    <TableRow>
                                        <TableCell align="left" className="font-bold text-green-800" sx={{ width: '25%' }}>
                                            Name
                                        </TableCell>
                                ...
                                <TableBody>
                                    {rows.filter(row => row.isShown).map((row) => ( // Only show rows where isShown is true
                                        <TableRow
                                            key={row.file_name}
                                        ...
                                        >
                                            ...
                                            <TableCell align="left" sx={{ width: '15%', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                {row.file_url}
                                            </TableCell>
                                            <TableCell align="left" sx={{ width: '15%' }}>
                                                <Button 
                                                    ...
                                                    color="error" 
                                                    size="small" 
                                                    onClick={() => handleDeleteLog(row.id)} // Call handleDeleteLog on click
                                                >
                                                    Delete Log
                                                </Button>
                                            </TableCell>
